...
class Trainer:
    ...
    def __init__(self, config):
        ...
        # Optional online-trained unidad_score. Config-gated by
        ...
        if dual_dmd_on:
            ...
            if self.is_main_process:
                print(
                    ...
                    f"real_score_weight={dd_cfg.real_score_weight}, "
                    f"unidad_score_weight={dd_cfg.unidad_score_weight}, "
                    f"update_ratio={dd_cfg.score_update_ratio}")

            # 1. Build unidad_score with student architecture (causal 1.3B).
            unidad_score = WanDiffusionWrapper(
                model_name=dd_cfg.score_model_name,
                is_causal=dd_cfg.score_is_causal,
                local_attn_size=dd_cfg.score_local_attn_size,
                sink_size=dd_cfg.score_sink_size,
            )
            unidad_score.model.requires_grad_(True)
            if config.gradient_checkpointing:
                unidad_score.enable_gradient_checkpointing()

            # 2. Build separate trainable MotionEncoder for unidad_score.
            me_cfg = config.motion_encoder
            unidad_motion_encoder = MotionEncoder(
            ...
            unidad_motion_encoder.requires_grad_(True)

            # 3. Initialize from same base ckpt as student (longlive_init.pt).
            init_ckpt_path = getattr(dd_cfg, "score_init_from", None) \
            ...
            if init_ckpt_path:
                ...
                if gen_key is not None:
                    unidad_score.load_state_dict(init_ckpt[gen_key], strict=False)
                if "motion_encoder" in init_ckpt:
                    unidad_motion_encoder.load_state_dict(
                        init_ckpt["motion_encoder"], strict=False)

            # 4. FSDP wrap — with grad (unlike real_score which is frozen).
            unidad_score = fsdp_wrap(
                unidad_score,
                sharding_strategy=config.sharding_strategy,
                mixed_precision=config.mixed_precision,
                wrap_strategy=dd_cfg.score_fsdp_wrap_strategy,
                cpu_offload=getattr(dd_cfg, "score_cpu_offload", False),
            )
            unidad_motion_encoder = unidad_motion_encoder.to(
                device=self.device, dtype=self.dtype)
            unidad_motion_encoder = fsdp_wrap(
                unidad_motion_encoder,
                sharding_strategy="no_shard",
                mixed_precision=config.mixed_precision,
                wrap_strategy="size",
                min_num_params=int(1e7),
            ...
            self.model.unidad_score = unidad_score
            self.model.unidad_motion_encoder = unidad_motion_encoder
            self.model.real_score_weight = float(dd_cfg.real_score_weight)
            self.model.unidad_score_weight = float(dd_cfg.unidad_score_weight)
            self.unidad_score_update_ratio = int(dd_cfg.score_update_ratio)

            # 5. Dedicated optimizer for unidad_score + unidad_motion_encoder.
            unidad_params = [p for p in unidad_score.parameters() if p.requires_grad]
            unidad_params += [p for p in unidad_motion_encoder.parameters() if p.requires_grad]
            self.unidad_score_optimizer = torch.optim.AdamW(
                unidad_params,
                lr=float(getattr(dd_cfg, "score_lr", config.lr)),
                betas=(
                    float(getattr(dd_cfg, "score_beta1", config.beta1)),
                    float(getattr(dd_cfg, "score_beta2", config.beta2)),
                ),
                weight_decay=config.weight_decay,
            )
        else:
            self.model.unidad_score = None
            self.model.unidad_motion_encoder = None
            self.model.real_score_weight = 1.0
            self.model.unidad_score_weight = 0.0
            self.unidad_score_update_ratio = 0
            self.unidad_score_optimizer = None
        ...
        # Uni-DAD: when unidad_score is enabled, ask MotionSwitchDataset for
        # a longer "target_video" clip (self-pair) for unidad_score_turn MSE.
        ...
        if self.unidad_score_optimizer is not None:
            lat_T = int(getattr(
    ...
    def save(self):
        ...
        if self.is_lora_enabled:
            ...
            # Uni-DAD: save unidad_score + its motion_encoder when enabled.
            unidad_score_sd = None
            unidad_motion_encoder_sd = None
            unidad_score_opt_sd = None
            if getattr(self.model, "unidad_score", None) is not None:
                with FSDP.state_dict_type(
                    self.model.unidad_score,
                    StateDictType.FULL_STATE_DICT,
                    FullStateDictConfig(rank0_only=True, offload_to_cpu=True),
                    FullOptimStateDictConfig(rank0_only=True),
                ):
                    unidad_score_sd = self.model.unidad_score.state_dict()
                    if self.unidad_score_optimizer is not None:
                        unidad_score_opt_sd = FSDP.optim_state_dict(
                            self.model.unidad_score,
                            self.unidad_score_optimizer)
                if getattr(self.model, "unidad_motion_encoder", None) is not None:
                    with FSDP.state_dict_type(
                        self.model.unidad_motion_encoder,
                        StateDictType.FULL_STATE_DICT,
                        FullStateDictConfig(rank0_only=True, offload_to_cpu=True),
                    ):
                        unidad_motion_encoder_sd = self.model.unidad_motion_encoder.state_dict()
            ...
            if unidad_score_sd is not None:
                state_dict["unidad_score"] = unidad_score_sd
            if unidad_motion_encoder_sd is not None:
                state_dict["unidad_motion_encoder"] = unidad_motion_encoder_sd
            if unidad_score_opt_sd is not None:
                state_dict["unidad_score_optimizer"] = unidad_score_opt_sd
    ...
    def fwdbwd_one_step_unidad_score_turn(self, batch):
        """Uni-DAD unidad_score_turn: one diffusion-MSE step on the unidad_score
        using self-pair data (prompt from video X, motion_ref + target_video
        also from video X). Updates unidad_score + unidad_motion_encoder.

        Mirrors the critic MSE mechanism but with:
          - opt  = unidad_score_optimizer
          - backbone = unidad_score (not fake_score)
          - grad path goes through unidad_motion_encoder (grad=True)
        """
        assert self.model.unidad_score is not None, (
            "unidad_score_turn requires unidad.dual_domain_dmd.enabled=true")
        assert "motion_ref" in batch, "unidad_score_turn needs motion_ref in batch"
        self.model.eval()
        ...
        # Motion tokens from unidad_motion_encoder WITH grad so it learns.
        motion_ref_dev = batch["motion_ref"].to(
            device=self.device, dtype=self.dtype)
        unidad_motion_tokens = self.model.unidad_motion_encoder(motion_ref_dev)
        cond = {
            **cond,
            "prompt_embeds": torch.cat(
                [text_embeds_no_motion, unidad_motion_tokens], dim=1),
        }
        ...
        with torch.no_grad():
            ...
            # In streaming mode, unidad_score sees chunks of latent length
        ...
        # Forward unidad_score with grad.
        flow_pred, pred_x0 = self.model.unidad_score(
        ...
        turn_loss = F.mse_loss(
            flow_pred.flatten(0, 1).float(),
            flow_target.float().detach(),
        )

        scaled_turn_loss = turn_loss / self.gradient_accumulation_steps
        scaled_turn_loss.backward()

        return {
            "unidad_score_turn_loss": turn_loss.detach(),
            "unidad_score_turn_grad_norm": torch.tensor(0.0, device=self.device),
        }
    ...
    def start_new_sequence(self):
        ...
        if motion_enabled:
            ...
            # using unidad_motion_encoder (separate frozen-at-this-step clone
            # for DMD target-branch forward; unidad_motion_encoder is updated
            # only on unidad_score_turn steps, not on every generator step).
            if getattr(self.model, "unidad_motion_encoder", None) is not None:
                motion_ref_dev = batch["motion_ref"].to(
                    device=self.device, dtype=self.dtype)
                with torch.no_grad():
                    unidad_motion_tokens = self.model.unidad_motion_encoder(
                        motion_ref_dev)
                conditional_dict_unidad_score = {
                    **conditional_dict,
                    "prompt_embeds": torch.cat(
                        [text_embeds_no_motion, unidad_motion_tokens], dim=1),
                }
                self.streaming_model.state["conditional_dict_unidad_score"] = (
                    conditional_dict_unidad_score)
    ...
    def _refresh_motion_in_state(self):
        ...
        # Uni-DAD: refresh target-side motion tokens (no_grad — unidad_motion_encoder
        # is updated separately on unidad_score_turn steps, not via this path).
        if (getattr(self.model, "unidad_motion_encoder", None) is not None
                and state.get("motion_ref_cpu") is not None):
            motion_ref = state["motion_ref_cpu"].to(
                device=self.device, dtype=self.dtype)
            with torch.no_grad():
                unidad_motion_tokens = self.model.unidad_motion_encoder(motion_ref)
            state["conditional_dict_unidad_score"] = {
                **cond_dict,
                "prompt_embeds": torch.cat(
                    [text_embeds, unidad_motion_tokens], dim=1),
            }
    ...
    def fwdbwd_one_step_streaming(self, train_generator):
        ...
        if train_generator:
            ...
            # source (real_score) and target (unidad_score) MSE branches.
            generator_loss, generator_log_dict = self.streaming_model.compute_generator_loss(
                chunk=generated_chunk,
                chunk_info=chunk_info,
                conditional_dict_unidad_score=self.streaming_model.state.get(
                    "conditional_dict_unidad_score"),
            )
    ...
    def _run_unidad_score_turn(self):
        """Run one accumulated unidad_score_turn update. Returns the merged log dict
        (empty when Uni-DAD is disabled)."""
        if self.unidad_score_optimizer is None:
            return {}
        if self.unidad_score_update_ratio <= 0:
            return {}
        if self.step % self.unidad_score_update_ratio != 0:
            return {}

        self.unidad_score_optimizer.zero_grad(set_to_none=True)
        accumulated = []
        for _ in range(self.gradient_accumulation_steps):
            unidad_batch = next(self.dataloader)
            accumulated.append(self.fwdbwd_one_step_unidad_score_turn(unidad_batch))
        unidad_grad_norm = self.model.unidad_score.clip_grad_norm_(
            self.max_grad_norm_generator)
        self.unidad_score_optimizer.step()
        unidad_log = merge_dict_list(accumulated)
        unidad_log["unidad_score_turn_grad_norm"] = unidad_grad_norm
        return unidad_log
        ...
        try:
            while True:
                ...
                if self.streaming_training:
                    ...
                    # Uni-DAD unidad_score_turn (gated inside the helper).
                    unidad_log_dict = self._run_unidad_score_turn()
                ...
                if self.is_main_process:
                    ...
                    if unidad_log_dict:
                        if "unidad_score_turn_loss" in unidad_log_dict:
                            wandb_loss_dict["unidad_score_turn_loss"] = (
                                unidad_log_dict["unidad_score_turn_loss"].mean().item())
                        if "unidad_score_turn_grad_norm" in unidad_log_dict:
                            wandb_loss_dict["unidad_score_turn_grad_norm"] = (
                                unidad_log_dict["unidad_score_turn_grad_norm"].mean().item())
                    # Surface dual-DMD extra term when unidad_score is active.
                    if TRAIN_GENERATOR and generator_log_dict:
                ...
                if self.is_main_process:
                    ...
                    if TRAIN_GENERATOR and generator_log_dict:
                        if "dmd_loss" in generator_log_dict:
                            dmd_decomp_str += (
                                f", dmd {generator_log_dict['dmd_loss'].mean().item():.4f}")
                        if "unidad_score_dmd_loss" in generator_log_dict:
                            dmd_decomp_str += (
