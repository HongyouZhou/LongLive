#!/bin/bash
#SBATCH --job-name=motion_equivariant_ram_train
...
# 20-outer × 16-inner training (~30 min at K=4) + inline motion_eval + vbench.
#SBATCH --mem=900G
#SBATCH --time=04:00:00
#SBATCH --output=logs/%x-%j.out
#SBATCH --exclude=s-sc-dgx[01-02]
#
# Motion-Equivariant RAM finetune on the few-step LongLive base.  It keeps
...
#
#   source scripts/hpc/submit.sh sbatch_motion_equivariant_ram_train.sh
#
#   # Override config (smoke = 2 outer × 4 inner × K=2):
#   LL_MERAM_CONFIG=longlive/methods/motion_equivariant_ram/configs/skateboarding_motion_equivariant_ram_smoke.yaml \
#     source scripts/hpc/submit.sh sbatch_motion_equivariant_ram_train.sh
#
#   LL_MERAM_SMOKE=1 source scripts/hpc/submit.sh sbatch_motion_equivariant_ram_train.sh
#
#   # Disable post-train eval:
#   LL_MERAM_EVAL=0 source scripts/hpc/submit.sh sbatch_motion_equivariant_ram_train.sh
#
# Pin GPU type when queue allows:
#   sbatch --gres=gpu:nvidia_h200:8        scripts/hpc/sbatch_motion_equivariant_ram_train.sh
#   sbatch --gres=gpu:nvidia_a100-sxm4:8   scripts/hpc/sbatch_motion_equivariant_ram_train.sh
...
mkdir -p "$PROJECT_DIR/logs" "$LL_DATA/motion_equivariant_ram_runs"
...
# Run
##############################
: "${LL_MERAM_CONFIG:=longlive/methods/motion_equivariant_ram/configs/skateboarding_motion_equivariant_ram.yaml}"
echo "[SLURM] config:  $LL_MERAM_CONFIG"

EXTRA_ARGS=()
if [ -n "${LL_MERAM_SMOKE:-}" ]; then
    EXTRA_ARGS+=(--smoke)
...
torchrun \
    ...
    -m longlive.methods.motion_equivariant_ram.train \
    --config "$LL_MERAM_CONFIG" \
    "${EXTRA_ARGS[@]}"
...
if [ "${LL_MERAM_EVAL:-1}" = "0" ]; then
    echo "[SLURM] LL_MERAM_EVAL=0 — skipping post-training eval."
    echo "[SLURM] Job finished."
    exit 0
fi
...
cfg = OmegaConf.load('$LL_MERAM_CONFIG')
...
CONFIG_BASENAME=$(basename "$LL_MERAM_CONFIG" .yaml)
EVAL_PREFIX="${LL_MERAM_RUN_PREFIX:-$CONFIG_BASENAME}"

echo "[SLURM] Post-train eval"
echo "[SLURM]   ckpt   = $CKPT"
echo "[SLURM]   prefix = $EVAL_PREFIX"

: "${LL_MERAM_VBENCH_CONFIG:=configs/vbench_short_motion_equivariant_ram.yaml}"
: "${LL_MERAM_EVAL_DATASETS:=ucf,loveu}"
: "${LL_MERAM_EVAL_GPUS:=0,1,2,3,4,5,6,7}"

EVAL_LIMIT_ARGS=()
if [ -n "${LL_MERAM_EVAL_LIMIT:-}" ]; then
    EVAL_LIMIT_ARGS+=(--limit "$LL_MERAM_EVAL_LIMIT")
fi
...
bash "$PROJECT_DIR/scripts/motion_eval/run_motion_eval.sh" \
    "$CKPT" "configs/motion_eval_inference_motion_equivariant_ram.yaml" "$RUN_ID_MOTION" \
    --gpus "$LL_MERAM_EVAL_GPUS" \
    --datasets "$LL_MERAM_EVAL_DATASETS" \
    "${EVAL_LIMIT_ARGS[@]}"
RC_MOTION=$?
...
if [ ! -f "$VBENCH_INFO" ]; then
    ...
    bash "$PROJECT_DIR/scripts/vbench/run_vbench.sh" \
        "$CKPT" "$LL_MERAM_VBENCH_CONFIG" "$RUN_ID_VBENCH" \
        --gpus "$LL_MERAM_EVAL_GPUS" \
        "${EVAL_LIMIT_ARGS[@]}"
    RC_VBENCH=$?
fi
