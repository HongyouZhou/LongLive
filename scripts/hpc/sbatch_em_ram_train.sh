...
#
# EM-RAM finetune on the few-step LongLive base.  The E-step builds a
# reward-tilted endpoint distribution under a KL budget; the M-step uses RAM's
# residual target geometry.
#
# Usage (always via submit.sh wrapper; captures $JID):
#
#   source scripts/hpc/submit.sh sbatch_em_ram_train.sh
#
#   # Override config (smoke = 2 outer x 4 inner x K=2):
#   LL_EM_RAM_CONFIG=longlive/methods/em_ram/configs/skateboarding_em_ram_smoke.yaml \
#     source scripts/hpc/submit.sh sbatch_em_ram_train.sh
#
#   LL_EM_RAM_SMOKE=1 source scripts/hpc/submit.sh sbatch_em_ram_train.sh
#
#   # Disable post-train eval:
#   LL_EM_RAM_EVAL=0 source scripts/hpc/submit.sh sbatch_em_ram_train.sh
#
# Pin GPU type when queue allows:
#   sbatch --gres=gpu:nvidia_h200:8        scripts/hpc/sbatch_em_ram_train.sh
#   sbatch --gres=gpu:nvidia_a100-sxm4:8   scripts/hpc/sbatch_em_ram_train.sh
...
mkdir -p "$PROJECT_DIR/logs" "$LL_DATA/em_ram_runs"
...
# Run
##############################
: "${LL_EM_RAM_CONFIG:=longlive/methods/em_ram/configs/skateboarding_em_ram.yaml}"
echo "[SLURM] config:  $LL_EM_RAM_CONFIG"

EXTRA_ARGS=()
if [ -n "${LL_EM_RAM_SMOKE:-}" ]; then
    EXTRA_ARGS+=(--smoke)
...
torchrun \
    ...
    -m longlive.methods.em_ram.train \
    --config "$LL_EM_RAM_CONFIG" \
    "${EXTRA_ARGS[@]}"
...
if [ "${LL_EM_RAM_EVAL:-1}" = "0" ]; then
    echo "[SLURM] LL_EM_RAM_EVAL=0 - skipping post-training eval."
    echo "[SLURM] Job finished."
    exit 0
fi
...
cfg = OmegaConf.load('$LL_EM_RAM_CONFIG')
...
CONFIG_BASENAME=$(basename "$LL_EM_RAM_CONFIG" .yaml)
EVAL_PREFIX="${LL_EM_RAM_RUN_PREFIX:-$CONFIG_BASENAME}"

echo "[SLURM] Post-train eval"
echo "[SLURM]   ckpt   = $CKPT"
echo "[SLURM]   prefix = $EVAL_PREFIX"

: "${LL_EM_RAM_VBENCH_CONFIG:=configs/vbench_short_em_ram.yaml}"
: "${LL_EM_RAM_EVAL_DATASETS:=ucf,loveu}"
: "${LL_EM_RAM_EVAL_GPUS:=0,1,2,3,4,5,6,7}"

EVAL_LIMIT_ARGS=()
if [ -n "${LL_EM_RAM_EVAL_LIMIT:-}" ]; then
    EVAL_LIMIT_ARGS+=(--limit "$LL_EM_RAM_EVAL_LIMIT")
fi
...
bash "$PROJECT_DIR/scripts/motion_eval/run_motion_eval.sh" \
    "$CKPT" "configs/motion_eval_inference_em_ram.yaml" "$RUN_ID_MOTION" \
    --gpus "$LL_EM_RAM_EVAL_GPUS" \
    --datasets "$LL_EM_RAM_EVAL_DATASETS" \
    "${EVAL_LIMIT_ARGS[@]}"
RC_MOTION=$?
...
if [ ! -f "$VBENCH_INFO" ]; then
    ...
    bash "$PROJECT_DIR/scripts/vbench/run_vbench.sh" \
        "$CKPT" "$LL_EM_RAM_VBENCH_CONFIG" "$RUN_ID_VBENCH" \
        --gpus "$LL_EM_RAM_EVAL_GPUS" \
        "${EVAL_LIMIT_ARGS[@]}"
    RC_VBENCH=$?
fi
