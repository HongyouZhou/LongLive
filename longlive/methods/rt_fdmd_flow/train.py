"""RT-fDMD-Flow trainer for the 4-step DMD base.

Single-file trainer implementing reward-tilted flow-map distillation on top of
`longlive_base.pt` + NVlabs `lora.pt` merged base.  It is structurally derived
from `longlive/methods/diffusion_ram/train.py`, but changes the objective:
reward is a bounded density-ratio tilt and an explicit local flow-map penalty
keeps the student transition close to the frozen base transition.
...
Outer loop (per epoch):
  ...
  3. Cross-rank all_gather over K x world_size rewards, group-normalize -> r in [0, 1].
  4. set_adapter("default");  for `inner_steps` gradient updates:
       ...
       f. loss = reward_tilted_flow_loss(...), optionally plus beta_kl anchor.
       g. backward, optimizer.step().
  5. Save LoRA ckpt every `ckpt_interval` outer epochs.

Differences from RAM:
  * target is a convex tilt between frozen-base velocity and FM velocity,
    rather than RAM's raw reward-scaled target shift.
  * default tilt mode is bounded JS-style rho/(1+rho), with clipped rho.
  * local one-step flow-map consistency regularizes the student transition.
"""
...
from longlive.methods.rt_fdmd_flow.losses import (
    kl_anchor_loss,
    reward_tilted_flow_loss,
)
...
def main():
    ...
    if args.smoke:
        ...
        # The grouped-noising assertion requires inner_steps == k_noisings x g_endpoints
    ...
    # Startup invariant - we group K_noisings under each rollout endpoint,
    ...
    assert int(cfg.k_rollouts) == g_endpoints, (
        f"RT-fDMD-Flow requires k_rollouts == g_endpoints_per_outer; got "
    ...
    if wandb_enabled:
        ...
        wandb.init(
            project=getattr(cfg, "wandb_project", "longlive_rt_fdmd_flow"),
            entity=getattr(cfg, "wandb_entity", "hongyou"),
            name=run_name,
            config=OmegaConf.to_container(cfg, resolve=True),
            dir=os.environ.get("WANDB_DIR", "wandb"),
        )
        print(f"[rt_fdmd_flow] wandb run: {wandb.run.url}", flush=True)
    ...
    # ---------- VAE ----------
    if rank0:
        print("[rt_fdmd_flow] loading VAE (bf16) ...", flush=True)
    ...
    if rank0:
        print("[rt_fdmd_flow] loading text encoder ...", flush=True)
    ...
    if rank0:
        ...
        print(f"[rt_fdmd_flow] building {cfg.model_name} ({arch}) ...", flush=True)
    model_kwargs = dict(
    ...
    if rank0:
        print(f"[rt_fdmd_flow] loading base ckpt: {base_ckpt_path}", flush=True)
    ...
    if rank0:
        print(
            f"[rt_fdmd_flow] base load: missing={len(missing)} unexpected={len(unexpected)}",
            flush=True,
        )
    del sd, state
    ...
    if baseline_lora_ckpt:
        ...
        if rank0:
            print(
                f"[rt_fdmd_flow] overlaying NVlabs baseline LoRA: {baseline_lora_ckpt}",
                flush=True,
            )
        ...
        if rank0:
            print("[rt_fdmd_flow] baseline LoRA merged into base weights", flush=True)
        del baseline_state
    ...
    if rank0:
        print("[rt_fdmd_flow] attaching adapters: default + anchor (v_ref)", flush=True)
    generator.model = configure_adapter_for_model(
        generator.model,
        model_name="generator",
    ...
    for p in generator.model.parameters():
        if p.dtype == torch.float32:
            ...
            n_cast += 1
    if rank0:
        print(f"[rt_fdmd_flow] cast {n_cast} fp32 params -> bf16 (post-LoRA, pre-FSDP)", flush=True)
    ...
    if rank0:
        print(
            f"[rt_fdmd_flow] adapter param counts: "
    ...
    if rank0:
        print(
            f"[rt_fdmd_flow] trainable params (FSDP-sharded total): "
    ...
    if rank0:
        print(f"[rt_fdmd_flow] init reward (rank 0 first): ref={ref_clip_path}", flush=True)
    ...
    dist.barrier()
    if rank0:
        print("[rt_fdmd_flow] reward init complete on all ranks", flush=True)
    ...
    reward_beta = float(getattr(cfg, "reward_beta", 2.0))
    reward_baseline = float(getattr(cfg, "reward_baseline", 0.5))
    tilt_mode = str(getattr(cfg, "tilt_mode", "js"))
    ratio_clip_min = float(getattr(cfg, "ratio_clip_min", 0.25))
    ratio_clip_max = float(getattr(cfg, "ratio_clip_max", 4.0))
    lambda_flowmap = float(getattr(cfg, "lambda_flowmap", 0.05))
    ...
    if rank0:
        print(
            f"[rt_fdmd_flow] start: outer={outer_epochs} x inner={inner_steps} "
            f"x K={K} (g_endpoints={g_endpoints}, k_noisings={k_noisings}) | "
            f"tilt={tilt_mode} beta={reward_beta} baseline={reward_baseline} "
            f"clip=[{ratio_clip_min}, {ratio_clip_max}] | "
            f"reward_coef={reward_coef} | lambda_flowmap={lambda_flowmap} "
            f"lambda_anchor={lambda_anchor} | beta_kl={beta_kl} | "
    ...
    if rank0:
        print(f"[rt_fdmd_flow] setup_time_s={setup_time_s:.1f}", flush=True)
    ...
    for outer in range(outer_epochs):
        ...
        for inner in range(inner_steps):
            ...
            # --- loss ---
            loss_rt, diag = reward_tilted_flow_loss(
                ...
                lambda_flowmap=lambda_flowmap,
                lambda_anchor=lambda_anchor,
                anchor_idx=t_idx,
            )
            if beta_kl > 0.0:
                kl = kl_anchor_loss(flow_pred_default, flow_pred_anchor)
                loss = loss_rt + beta_kl * kl
            else:
                kl = torch.zeros((), device=device, dtype=loss_rt.dtype)
                loss = loss_rt
            ...
            sum_tilt_loss += float(diag["loss/tilt"])
            sum_flowmap_loss += float(diag["loss/flowmap"])
            sum_anchor_loss += float(diag["loss/anchor"])
            sum_ratio += float(diag["rt/ratio"])
            sum_tilt_weight += float(diag["rt/tilt_weight"])
            sum_alpha += float(diag["rt/alpha"])
            sum_target_norm += float(diag["rt/target_norm"])
            sum_flow_target_norm += float(diag["rt/flow_target_norm"])
            sum_v_default_norm += float(diag["rt/v_default_norm"])
            sum_v_anchor_norm += float(diag["rt/v_anchor_norm"])
            sum_r_scalar += float(diag["rt/r_scalar"])
            per_anchor_alpha[t_idx] += float(diag["rt/alpha"])
            per_anchor_count[t_idx] += 1
            per_rollout_alpha[k_idx] += float(diag["rt/alpha"])
        ...
        avg_loss = sum_loss / n
        avg_kl = sum_kl / n
        avg_tilt_loss = sum_tilt_loss / n
        avg_flowmap_loss = sum_flowmap_loss / n
        avg_anchor_loss = sum_anchor_loss / n
        avg_ratio = sum_ratio / n
        avg_tilt_weight = sum_tilt_weight / n
        avg_alpha = sum_alpha / n
        avg_target = sum_target_norm / n
        avg_flow_target = sum_flow_target_norm / n
        avg_v_default = sum_v_default_norm / n
        avg_v_anchor = sum_v_anchor_norm / n
        avg_r = sum_r_scalar / n
        if rank0:
            print(
                f"[rt_fdmd_flow] outer {outer:3d}/{outer_epochs}  "
                f"rewards mean={reward_diag['reward/raw_mean']:.3f} "
                f"std={reward_diag['reward/raw_std']:.3f}  "
                f"loss={avg_loss:.4f}  r_mean={avg_r:.3f}  "
                f"alpha={avg_alpha:.3f}  ratio={avg_ratio:.3f}  "
                f"flowmap={avg_flowmap_loss:.4f}  "
        ...
        if wandb_enabled:
            log_dict = {
                "outer/loss": avg_loss,
                "outer/loss_tilt": avg_tilt_loss,
                "outer/loss_flowmap": avg_flowmap_loss,
                "outer/loss_anchor": avg_anchor_loss,
                "outer/ratio": avg_ratio,
                "outer/tilt_weight": avg_tilt_weight,
                "outer/alpha": avg_alpha,
                "outer/target_norm": avg_target,
                "outer/flow_target_norm": avg_flow_target,
            ...
            # Per-anchor / per-rollout effective tilt.
            for t_i, t_v in enumerate(anchors):
                if per_anchor_count[t_i]:
                    log_dict[f"rt/alpha_t{t_i}"] = (
                        per_anchor_alpha[t_i] / per_anchor_count[t_i]
                    )
            for k_i in range(K):
                if per_rollout_count[k_i]:
                    log_dict[f"rt/alpha_k{k_i}"] = (
                        per_rollout_alpha[k_i] / per_rollout_count[k_i]
                    )
        ...
        if (
            ...
            if rank0:
                ...
                print(f"[rt_fdmd_flow] saved ckpt: {ckpt_path}", flush=True)
    ...
    if rank0:
        print(
            f"[rt_fdmd_flow] DONE. setup_time_s={setup_time_s:.1f} "
