"""Motion-Equivariant RAM trainer for the 4-step DMD base.
...
loss changes the reward update subspace so that high-reward endpoints update
coarse temporal motion only; static/appearance velocity stays base-anchored.
...
Outer loop (per epoch):
  ...
  4. set_adapter("default");  for `inner_steps` gradient updates:
       ...
       f. loss = motion_equivariant_ram_loss(...), where r_k gates only
          coarse temporal-delta x0 motion and static velocity is base-anchored.
...
from longlive.methods.motion_equivariant_ram.losses import (
    kl_anchor_loss,
    motion_equivariant_ram_loss,
)
from longlive.methods.motion_equivariant_ram.reward import MotionEquivariantReward
...
def main():
    ...
    if rank0:
        print("[motion_equivariant_ram] resolved config:")
        print(OmegaConf.to_yaml(cfg))
        gpu_name = torch.cuda.get_device_name(device)
        gpu_total_gib = torch.cuda.get_device_properties(device).total_memory / 1024 ** 3
        print(
            f"[motion_equivariant_ram] device: {gpu_name} ({gpu_total_gib:.1f} GiB) "
    ...
    if wandb_enabled:
        ...
        wandb.init(
            project=getattr(cfg, "wandb_project", "longlive_motion_equivariant_ram"),
            entity=getattr(cfg, "wandb_entity", "hongyou"),
            name=run_name,
            config=OmegaConf.to_container(cfg, resolve=True),
            dir=os.environ.get("WANDB_DIR", "wandb"),
        )
        print(f"[motion_equivariant_ram] wandb run: {wandb.run.url}", flush=True)
    ...
    # ---------- VAE ----------
    if rank0:
        print("[motion_equivariant_ram] loading VAE (bf16) ...", flush=True)
    ...
    if rank0:
        print("[motion_equivariant_ram] loading text encoder ...", flush=True)
    ...
    if rank0:
        ...
        print(f"[motion_equivariant_ram] building {cfg.model_name} ({arch}) ...", flush=True)
    model_kwargs = dict(
    ...
    if rank0:
        print(f"[motion_equivariant_ram] loading base ckpt: {base_ckpt_path}", flush=True)
    ...
    if rank0:
        print(
            f"[motion_equivariant_ram] base load: missing={len(missing)} unexpected={len(unexpected)}",
            flush=True,
        )
    del sd, state
    ...
    if baseline_lora_ckpt:
        ...
        if rank0:
            print(
                f"[motion_equivariant_ram] overlaying NVlabs baseline LoRA: {baseline_lora_ckpt}",
                flush=True,
            )
        ...
        if rank0:
            print("[motion_equivariant_ram] baseline LoRA merged into base weights", flush=True)
        del baseline_state
    ...
    if rank0:
        print("[motion_equivariant_ram] attaching adapters: default + anchor (v_ref)", flush=True)
    generator.model = configure_adapter_for_model(
        generator.model,
        model_name="generator",
    ...
    for p in generator.model.parameters():
        if p.dtype == torch.float32:
            ...
            n_cast += 1
    if rank0:
        print(f"[motion_equivariant_ram] cast {n_cast} fp32 params → bf16 (post-LoRA, pre-FSDP)", flush=True)
    ...
    if rank0:
        print(
            f"[motion_equivariant_ram] adapter param counts: "
    ...
    if rank0:
        print(
            f"[motion_equivariant_ram] trainable params (FSDP-sharded total): "
    ...
    if rank0:
        print(f"[motion_equivariant_ram] init reward (rank 0 first): ref={ref_clip_path}", flush=True)
        reward_fn = MotionEquivariantReward(
    ...
    dist.barrier()
    if rank0:
        print("[motion_equivariant_ram] reward init complete on all ranks", flush=True)
    ...
    if rank0:
        print(
            f"[motion_equivariant_ram] start: outer={outer_epochs} × inner={inner_steps} "
    ...
    if rank0:
        print(f"[motion_equivariant_ram] setup_time_s={setup_time_s:.1f}", flush=True)
    ...
    for outer in range(outer_epochs):
        ...
        for inner in range(inner_steps):
            ...
            # --- loss ---
            loss_meram, diag = motion_equivariant_ram_loss(
            ...
            if beta_kl > 0.0:
                ...
                loss = loss_meram + beta_kl * kl
            else:
                kl = torch.zeros((), device=device, dtype=loss_meram.dtype)
                loss = loss_meram
            ...
            sum_motion_loss += float(diag["meram/motion_loss"])
            sum_static_loss += float(diag["meram/static_loss"])
            sum_delta_default_norm += float(diag["meram/delta_default_norm"])
            sum_delta_target_norm += float(diag["meram/delta_target_norm"])
            sum_v_default_norm += float(diag["meram/v_default_norm"])
            sum_v_anchor_norm += float(diag["meram/v_anchor_norm"])
            sum_r_scalar += float(diag["meram/r_scalar"])
            sum_motion_weight += float(diag["meram/motion_weight"])
            per_anchor_delta[t_idx] += float(diag["meram/delta_default_norm"])
            per_anchor_count[t_idx] += 1
            per_rollout_delta[k_idx] += float(diag["meram/delta_default_norm"])
        ...
        if rank0:
            print(
                f"[motion_equivariant_ram] outer {outer:3d}/{outer_epochs}  "
        ...
        if wandb_enabled:
            ...
            for t_i, t_v in enumerate(anchors):
                if per_anchor_count[t_i]:
                    log_dict[f"meram/delta_default_norm_t{t_i}"] = (
                        per_anchor_delta[t_i] / per_anchor_count[t_i]
                    )
            for k_i in range(K):
                if per_rollout_count[k_i]:
                    log_dict[f"meram/delta_default_norm_k{k_i}"] = (
        ...
        if (
            ...
            if rank0:
                ...
                print(f"[motion_equivariant_ram] saved ckpt: {ckpt_path}", flush=True)
    ...
    if rank0:
        print(
            f"[motion_equivariant_ram] DONE. setup_time_s={setup_time_s:.1f} "
