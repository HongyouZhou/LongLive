"""EM-RAM trainer for the 4-step DMD base.
...
Outer loop (per epoch):
  ...
  4. set_adapter("default");  for `inner_steps` gradient updates:
       ...
       f. loss = em_ram_loss(v_default, v_anchor, eps, x_0_k, alpha_k, reward_coef).
...
Differences from NFT:
  ...
  * Logging drops `outer/kl`, `outer/decay`, `outer/smooth_neg_over_default`;
    adds `outer/{shift_norm, target_norm, v_default_norm, v_anchor_norm, alpha_mean}`
...
from longlive.methods.em_ram.losses import em_ram_loss, em_tilt_weights, kl_anchor_loss
...
def main():
    ...
    assert int(cfg.k_rollouts) == g_endpoints, (
        f"EM-RAM requires k_rollouts == g_endpoints_per_outer; got "
    ...
    if wandb_enabled:
        ...
        wandb.init(
            project=getattr(cfg, "wandb_project", "longlive_em_ram"),
            entity=getattr(cfg, "wandb_entity", "hongyou"),
            name=run_name,
            config=OmegaConf.to_container(cfg, resolve=True),
            dir=os.environ.get("WANDB_DIR", "wandb"),
        )
        print(f"[em_ram] wandb run: {wandb.run.url}", flush=True)
    ...
    # ---------- VAE ----------
    if rank0:
        print("[em_ram] loading VAE (bf16) ...", flush=True)
    ...
    if rank0:
        print("[em_ram] loading text encoder ...", flush=True)
    ...
    if rank0:
        ...
        print(f"[em_ram] building {cfg.model_name} ({arch}) ...", flush=True)
    model_kwargs = dict(
    ...
    if rank0:
        print(f"[em_ram] loading base ckpt: {base_ckpt_path}", flush=True)
    ...
    if rank0:
        print(
            f"[em_ram] base load: missing={len(missing)} unexpected={len(unexpected)}",
            flush=True,
        )
    del sd, state
    ...
    if baseline_lora_ckpt:
        ...
        if rank0:
            print(
                f"[em_ram] overlaying NVlabs baseline LoRA: {baseline_lora_ckpt}",
                flush=True,
            )
        ...
        if rank0:
            print("[em_ram] baseline LoRA merged into base weights", flush=True)
        del baseline_state
    ...
    for p in generator.model.parameters():
        if p.dtype == torch.float32:
            ...
            n_cast += 1
    if rank0:
        print(f"[em_ram] cast {n_cast} fp32 params -> bf16 (post-LoRA, pre-FSDP)", flush=True)
    ...
    if rank0:
        print(
            f"[em_ram] trainable params (FSDP-sharded total): "
    ...
    if rank0:
        print(f"[em_ram] init reward (rank 0 first): ref={ref_clip_path}", flush=True)
    ...
    dist.barrier()
    if rank0:
        print("[em_ram] reward init complete on all ranks", flush=True)
    ...
    if rank0:
        print(
            f"[em_ram] start: outer={outer_epochs} x inner={inner_steps} "
    ...
    if rank0:
        print(f"[em_ram] setup_time_s={setup_time_s:.1f}", flush=True)
    ...
    for outer in range(outer_epochs):
        ...
        for inner in range(inner_steps):
            # EM-RAM cycling: noisings grouped under endpoint, anchor rotates inside.
            ...
            if beta_kl > 0.0:
                ...
                loss = loss_em_ram + beta_kl * kl
            else:
                kl = torch.zeros((), device=device, dtype=loss_em_ram.dtype)
                loss = loss_em_ram
            ...
            sum_shift_norm += float(diag["em_ram/shift_norm"])
            sum_target_norm += float(diag["em_ram/target_norm"])
            sum_v_default_norm += float(diag["em_ram/v_default_norm"])
            sum_v_anchor_norm += float(diag["em_ram/v_anchor_norm"])
            sum_alpha += float(diag["em_ram/alpha"])
            sum_alpha_eff += float(diag["em_ram/alpha_eff"])
            per_anchor_shift[t_idx] += float(diag["em_ram/shift_norm"])
            per_anchor_count[t_idx] += 1
            per_rollout_shift[k_idx] += float(diag["em_ram/shift_norm"])
        ...
        if rank0:
            print(
                f"[em_ram] outer {outer:3d}/{outer_epochs}  "
        ...
        if wandb_enabled:
            ...
            for t_i, t_v in enumerate(anchors):
                if per_anchor_count[t_i]:
                    log_dict[f"em_ram/shift_norm_t{t_i}"] = (
                        per_anchor_shift[t_i] / per_anchor_count[t_i]
                    )
            for k_i in range(K):
                if per_rollout_count[k_i]:
                    log_dict[f"em_ram/shift_norm_k{k_i}"] = (
        ...
        if (
            ...
            if rank0:
                ...
                print(f"[em_ram] saved ckpt: {ckpt_path}", flush=True)
    ...
    if rank0:
        print(
            f"[em_ram] DONE. setup_time_s={setup_time_s:.1f} "
