"""Motion-Equivariant RAM RefDelta trainer for the 4-step DMD base.

This is structurally forked from `longlive.methods.diffusion_ram.train`: same
rollout, reward, cross-rank group normalization, and adapter setup.  The inner
loss changes both the reward target and update subspace: high-reward endpoints
update toward the reference clip's coarse temporal motion, while static/
appearance velocity stays base-anchored.
...
Outer loop (per epoch):
  ...
  4. set_adapter("default");  for `inner_steps` gradient updates:
       ...
       c. (v_default, pred_x0_default) = generator(x_t, t), grad ON.
       d. set_adapter("anchor");  (v_anchor, pred_x0_anchor), no_grad.
       e. set_adapter("default") to restore before backward (gc safety).
       f. loss = motion_equivariant_ram_refdelta_loss(...), where r_k gates only
          reference coarse temporal-delta x0 motion and static velocity is
          base-anchored.
...
Differences from NFT:
  ...
  * `k_idx / t_idx` cycling groups noisings under an endpoint.
  * Logging reports motion/static loss, coarse-delta norms, and reward weight.
"""
...
from longlive.methods.motion_equivariant_ram_refdelta.losses import (
    kl_anchor_loss,
    motion_equivariant_ram_refdelta_loss,
)
from longlive.methods.motion_equivariant_ram_refdelta.reward import MotionEquivariantRefDeltaReward
...
class _VAEEncodeDtypeAdapter:
    """Cast dataset pixels to the frozen VAE compute dtype before encoding."""

    def __init__(self, vae: WanVAEWrapper, dtype: torch.dtype):
        self.vae = vae
        self.dtype = dtype

    def encode_to_latent(self, pixel: torch.Tensor) -> torch.Tensor:
        return self.vae.encode_to_latent(pixel.to(dtype=self.dtype))
...
def main():
    ...
    if rank0:
        print("[motion_equivariant_ram_refdelta] resolved config:")
        print(OmegaConf.to_yaml(cfg))
        gpu_name = torch.cuda.get_device_name(device)
        gpu_total_gib = torch.cuda.get_device_properties(device).total_memory / 1024 ** 3
        print(
            f"[motion_equivariant_ram_refdelta] device: {gpu_name} ({gpu_total_gib:.1f} GiB) "
    ...
    if wandb_enabled:
        ...
        wandb.init(
            project=getattr(cfg, "wandb_project", "longlive_motion_equivariant_ram_refdelta"),
            entity=getattr(cfg, "wandb_entity", "hongyou"),
            name=run_name,
            config=OmegaConf.to_container(cfg, resolve=True),
            dir=os.environ.get("WANDB_DIR", "wandb"),
        )
        print(f"[motion_equivariant_ram_refdelta] wandb run: {wandb.run.url}", flush=True)
    ...
    # ---------- VAE ----------
    if rank0:
        print("[motion_equivariant_ram_refdelta] loading VAE (bf16) ...", flush=True)
    vae = WanVAEWrapper()
    ...
    dataset = SkateboardingLatentDataset(
        ...
        vae=_VAEEncodeDtypeAdapter(vae, torch.bfloat16),
    ...
    if rank0:
        print("[motion_equivariant_ram_refdelta] encoding reference latent ...", flush=True)
    reference_latent, _ = dataset.sample()
    reference_latent = reference_latent.detach().to(device=device, dtype=torch.bfloat16)
    if rank0:
        print(
            f"[motion_equivariant_ram_refdelta] reference latent shape={tuple(reference_latent.shape)}",
            flush=True,
        )

    # ---------- Text encoder: load → encode → free ----------
    if rank0:
        print("[motion_equivariant_ram_refdelta] loading text encoder ...", flush=True)
    ...
    if rank0:
        ...
        print(f"[motion_equivariant_ram_refdelta] building {cfg.model_name} ({arch}) ...", flush=True)
    model_kwargs = dict(
    ...
    if rank0:
        print(f"[motion_equivariant_ram_refdelta] loading base ckpt: {base_ckpt_path}", flush=True)
    ...
    if rank0:
        print(
            f"[motion_equivariant_ram_refdelta] base load: missing={len(missing)} unexpected={len(unexpected)}",
            flush=True,
        )
    del sd, state
    ...
    if baseline_lora_ckpt:
        ...
        if rank0:
            print(
                f"[motion_equivariant_ram_refdelta] overlaying NVlabs baseline LoRA: {baseline_lora_ckpt}",
                flush=True,
            )
        ...
        if rank0:
            print("[motion_equivariant_ram_refdelta] baseline LoRA merged into base weights", flush=True)
        del baseline_state

    # ---------- Attach 2 PEFT adapters (BEFORE FSDP wrap) ----------
    # RAM only needs 2: "default" (trainable) + "anchor" (zero-init, frozen, = v_ref).
    # No "old" EMA adapter (NFT-specific).
    if rank0:
        print("[motion_equivariant_ram_refdelta] attaching adapters: default + anchor (v_ref)", flush=True)
    generator.model = configure_adapter_for_model(
        generator.model,
        model_name="generator",
    ...
    for p in generator.model.parameters():
        if p.dtype == torch.float32:
            ...
            n_cast += 1
    if rank0:
        print(f"[motion_equivariant_ram_refdelta] cast {n_cast} fp32 params → bf16 (post-LoRA, pre-FSDP)", flush=True)
    ...
    if rank0:
        print(
            f"[motion_equivariant_ram_refdelta] adapter param counts: "
    ...
    if rank0:
        print(
            f"[motion_equivariant_ram_refdelta] trainable params (FSDP-sharded total): "
    ...
    if rank0:
        print(f"[motion_equivariant_ram_refdelta] init reward (rank 0 first): ref={ref_clip_path}", flush=True)
        reward_fn = MotionEquivariantRefDeltaReward(
    ...
    dist.barrier()
    if rank0:
        print("[motion_equivariant_ram_refdelta] reward init complete on all ranks", flush=True)
    ...
    max_motion_weight = float(getattr(cfg, "max_motion_weight", 1.0))
    ...
    if rank0:
        print(
            f"[motion_equivariant_ram_refdelta] start: outer={outer_epochs} × inner={inner_steps} "
            f"× K={K} (g_endpoints={g_endpoints}, k_noisings={k_noisings}) | "
            f"reward_coef={reward_coef} | beta_kl={beta_kl} | "
            f"lambda_motion={lambda_motion} | lambda_static={lambda_static} | "
            f"motion_pool={motion_pool} | max_motion_weight={max_motion_weight} | "
    ...
    if rank0:
        print(f"[motion_equivariant_ram_refdelta] setup_time_s={setup_time_s:.1f}", flush=True)
    ...
    for outer in range(outer_epochs):
        ...
        for k_idx, (video, _latent) in enumerate(rollouts):
            ...
            reward_components.append(dict(getattr(reward_fn, "last_components", {})))
        t_reward = time.time() - t_reward_start
        reward_component_diag = {}
        if reward_components:
            keys = ("direction", "speed_penalty", "speed_ratio")
            for key in keys:
        ...
        sum_motion_loss = 0.0
        sum_static_loss = 0.0
        sum_delta_default_norm = 0.0
        sum_delta_target_norm = 0.0
        sum_v_default_norm = 0.0
        sum_v_anchor_norm = 0.0
        sum_r_scalar = 0.0
        sum_motion_weight = 0.0
        ...
        for inner in range(inner_steps):
            ...
            flow_pred_default, pred_x0_default = generator(noisy, train_cond, timestep)

            # --- anchor forward (no_grad) ---
            generator.model.set_adapter("anchor")
            with torch.no_grad():
                flow_pred_anchor, pred_x0_anchor = generator(noisy, train_cond, timestep)
            ...
            # --- loss ---
            loss_meref, diag = motion_equivariant_ram_refdelta_loss(
                v_default=flow_pred_default,
                v_anchor=flow_pred_anchor,
                pred_x0_default=pred_x0_default,
                pred_x0_anchor=pred_x0_anchor,
                ...
                max_motion_weight=max_motion_weight,
                anchor_idx=t_idx,
            )
            if beta_kl > 0.0:
                kl = kl_anchor_loss(flow_pred_default, flow_pred_anchor)
                loss = loss_meref + beta_kl * kl
            else:
                kl = torch.zeros((), device=device, dtype=loss_meref.dtype)
                loss = loss_meref
            ...
            sum_motion_loss += float(diag["meref/motion_loss"])
            sum_static_loss += float(diag["meref/static_loss"])
            sum_delta_default_norm += float(diag["meref/delta_default_norm"])
            sum_delta_target_norm += float(diag["meref/delta_target_norm"])
            sum_v_default_norm += float(diag["meref/v_default_norm"])
            sum_v_anchor_norm += float(diag["meref/v_anchor_norm"])
            sum_r_scalar += float(diag["meref/r_scalar"])
            sum_motion_weight += float(diag["meref/motion_weight"])
            per_anchor_delta[t_idx] += float(diag["meref/delta_default_norm"])
            per_anchor_count[t_idx] += 1
            per_rollout_delta[k_idx] += float(diag["meref/delta_default_norm"])
        ...
        avg_delta_default = sum_delta_default_norm / n
        avg_delta_target = sum_delta_target_norm / n
        avg_v_default = sum_v_default_norm / n
        avg_v_anchor = sum_v_anchor_norm / n
        avg_r = sum_r_scalar / n
        avg_motion_weight = sum_motion_weight / n
        if rank0:
            print(
                f"[motion_equivariant_ram_refdelta] outer {outer:3d}/{outer_epochs}  "
                ...
                f"static={avg_static_loss:.4f}  w={avg_motion_weight:.3f} "
                f"r_mean={avg_r:.3f}  "
                f"delta={avg_delta_default:.3f}->{avg_delta_target:.3f}  "
        ...
        if wandb_enabled:
            log_dict = {
                ...
                "outer/delta_default_norm": avg_delta_default,
                "outer/delta_target_norm": avg_delta_target,
                "outer/v_default_norm": avg_v_default,
                "outer/v_anchor_norm": avg_v_anchor,
                "outer/r_mean": avg_r,
                "outer/motion_weight": avg_motion_weight,
            ...
            for t_i, t_v in enumerate(anchors):
                if per_anchor_count[t_i]:
                    log_dict[f"meref/delta_default_norm_t{t_i}"] = (
                        per_anchor_delta[t_i] / per_anchor_count[t_i]
                    )
            for k_i in range(K):
                if per_rollout_count[k_i]:
                    log_dict[f"meref/delta_default_norm_k{k_i}"] = (
        ...
        if (
            ...
            if rank0:
                ...
                print(f"[motion_equivariant_ram_refdelta] saved ckpt: {ckpt_path}", flush=True)
    ...
    if rank0:
        print(
            f"[motion_equivariant_ram_refdelta] DONE. setup_time_s={setup_time_s:.1f} "
