...
class Trainer:
    ...
    def __init__(self, config):
        ...
        if self.is_main_process and not self.disable_wandb:
            ...
            # Persist wandb run id under logdir/wandb_run_id.txt so a
            # 48h-killed-then-resubmitted job continues the same wandb run
            # (otherwise wandb auto-creates a fresh run on every relaunch).
            run_id = None
            run_id_file = (os.path.join(config.logdir, "wandb_run_id.txt")
                           if config.logdir else None)
            if (getattr(config, "auto_resume", False)
                    and run_id_file and os.path.exists(run_id_file)):
            ...
            if run_id is not None:
                # Resume: skip `name` so wandb keeps the original run name
                # and don't conflict with the stored run.
                init_kwargs.update(id=run_id, resume="allow")
            else:
                init_kwargs["name"] = config.config_name
            wandb.init(**init_kwargs)

            # Write run id once, after wandb assigns it. Safe to overwrite
            # (resume path reads the same id back).
    ...
    def _attach_real_score_lora(self):
        """L5 extension hook (CLAUDE.md "L5 trainer phase: subclass
        ScoreDistillationTrainer; LoRA attach point ≈ line 350").
